...
import com.cloud.dips.common.core.exception.CheckedException;
import com.cloud.dips.common.core.exception.ValidateCodeException;
import com.cloud.dips.gateway.config.FilterIgnorePropertiesConfig;
...
public class ImageCodeGatewayFilter extends AbstractGatewayFilterFactory {
	...
	private void checkCode(ServerHttpRequest request) throws ValidateCodeException {
		...
		if (StrUtil.isBlank(randomStr)) {
			throw new ValidateCodeException("2");
		}

		String key = DEFAULT_CODE_KEY + randomStr;
		if (!redisTemplate.hasKey(key)) {
			throw new ValidateCodeException("3");
		}

		Object codeObj = redisTemplate.opsForValue().get(key);

		if (codeObj == null) {
			throw new ValidateCodeException("4");
		}

		String saveCode = codeObj.toString();
		if (StrUtil.isBlank(saveCode)) {
			redisTemplate.delete(key);
			throw new ValidateCodeException("5");
		}

		if (!StrUtil.equals(saveCode, code)) {
			redisTemplate.delete(key);
			throw new ValidateCodeException("6");
		}
