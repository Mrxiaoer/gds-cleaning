...
//		System.out.println("path:"+path);
		...
		if (analysisType == DataCleanConstant.QUICK_ANALYSIS) {
			//调用快速分析算法，采用层次聚类
//			arguments = new String[]{"/root/anaconda3/bin/python",
//				path + "/analysis_similarity/quickAnalyse.py", jsonFileUrl};
			arguments = new String[]{"/root/anaconda3/bin/python","/usr/local/quickAnalyse.py", jsonFileUrl};
...
//				path + "/analysis_similarity/deepAnalyse.py", jsonFileUrl};
			arguments = new String[]{"/root/anaconda3/bin/python","/usr/local/deepAnalyse.py", jsonFileUrl};
		}
		try {
			//调用python的相似度计算程序
...
//		String[] arguments = new String[]{"/root/anaconda3/bin/python",
//			path + "/analysis_similarity/centroidAnalyse.py", jsonStr};
		String[] arguments = new String[]{"/root/anaconda3/bin/python","/usr/local/centroidAnalyse.py", jsonStr};
		try {
			//调用python标准分析程序
			Process process = Runtime.getRuntime().exec(arguments);
			//将得到的数据进行io处理
